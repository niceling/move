...
function moveToUp(moveObj,callback){
	...
	if(callback==undefined || callback){
		if((!$upObj.find(".moveUp").length>0) && $upObj.find(".moveDown").length>0){//判读上一个元素是否是第一个元素
		...
		if((!tempObj.find(".moveDown").length>0)　&& tempObj.find(".moveUp").length>0){//判读当前元素是否是最后一个元素
...
function moveToDown(moveObj,callback){
	...
	var tempObj=$moveObj;
	if(callback==undefined || callback){
		if((!$nextObj.find(".moveDown").length>0) && $nextObj.find(".moveUp").length>0){//判读下一个元素是否是最后一个元素
		...
		if((!tempObj.find(".moveUp").length>0) && tempObj.find(".moveDown").length>0){//判读当前元素是否是第一个元素
